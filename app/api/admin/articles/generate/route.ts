...
- SEOを意識した構成
- 自然で親しみやすい、人が書いたような柔らかな口調で書く（AI臭を避ける）
- 見出し（H2, H3）を適切に使用
- 情報が整理しやすい場合は表（<table>タグ）を使って見やすくする
- 2000文字以上
- タイトルには${currentYear}年であることを明示
- 文末に文字数カウント（総文字数: 約○○文字）などのメタ情報は絶対に含めない

記事の形式（必ず以下の形式で出力してください）:
タイトル: [記事タイトル（${currentYear}年の情報であることを含める）]
メタディスクリプション: [SEO用の説明文、160文字以内]
本文: [HTML形式の記事本文。表が使える場合は積極的に使用。文字数カウントは不要。]`;
...
【最重要】
- 必ず${currentYear}年の最新情報を基に、読みやすく価値のある記事を作成してください
- 文章は自然で親しみやすく、まるで人が書いたかのような柔らかな口調で書いてください
- 機械的な表現やAI臭のある文章は避け、読者に寄り添った温かみのある表現を心がけてください
- 「〜如き」「〜べき」のような古い表現や堅苦しい表現は使わないでください
- 「〜してください」のような指示形を過度に使わず、情報提供型の自然な文章にしてください
- 文末に総文字数などのメタ情報は絶対に含めないでください
- 箇条書きは適度に使用し、文章の流れを重視してください
- 過去の予想や古い情報は使用せず、現在利用可能な最新のデータと情報のみを使用してください`,
          },
          {
            role: 'user',
            content: prompt,
