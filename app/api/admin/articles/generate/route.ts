    ...
    const grokResponse = await fetch('https://api.x.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `あなたはSEOに強い記事作成の専門家です。現在は${currentYear}年${currentMonth}月です。必ず${currentYear}年の最新情報を基に、読みやすく価値のある記事を作成してください。過去の予想や古い情報は使用せず、現在利用可能な最新のデータと情報のみを使用してください。文章は自然で親しみやすく、まるで人が書いたかのような柔らかな口調で書いてください。機械的な表現やAI臭のある文章は避け、読者に寄り添った温かみのある表現を心がけてください。また、文末に総文字数などのメタ情報は絶対に含めないでください。`,
          },
          {
            role: 'user',
            content: prompt,
