...
export default function ThemePage() {
  ...
  const getGlobalMenuDesign = (): GlobalMenuDesign => ({
    ...DEFAULT_GLOBAL_MENU_DESIGN,
    ...theme.menuSettings?.globalMenuDesign,
  });

  const updateGlobalMenuDesign = (key: keyof GlobalMenuDesign, value: any) => {
    setTheme(prev => ({
      ...prev,
      menuSettings: {
        ...prev.menuSettings,
        ...
        globalNavItems: prev.menuSettings?.globalNavItems || [],
        globalMenuDesign: {
          ...DEFAULT_GLOBAL_MENU_DESIGN,
          ...prev.menuSettings?.globalMenuDesign,
          [key]: value,
        },
      },
    }));
            ...
            <div className="p-8">
              ...
              {activeTab === 'menu' && (
                <div className="space-y-6">
                  ...
                  {/* グローバルメニュー デザイン設定 */}
                  {(theme.menuSettings?.globalNavItems || []).length > 0 && (
                    <div className="mt-8 pt-6 border-t border-green-200">
                      ...
                      {/* ホバーエフェクト */}
                      <div className="mb-5">
                        <label className="block text-xs font-medium text-gray-600 mb-2">ホバーエフェクト</label>
                        <div className="flex gap-2">
                          {([
                            { value: 'grayscale' as GlobalMenuHoverEffect, label: 'グレースケール' },
                            { value: 'darken' as GlobalMenuHoverEffect, label: '暗くする' },
                            { value: 'zoom' as GlobalMenuHoverEffect, label: 'ズーム' },
                            { value: 'none' as GlobalMenuHoverEffect, label: 'なし' },
                          ]).map(opt => (
                            <button
                              key={opt.value}
                              type="button"
                              onClick={() => updateGlobalMenuDesign('hoverEffect', opt.value)}
                              className={`flex-1 py-2 px-3 rounded-lg text-xs font-medium border transition-colors ${
                                getGlobalMenuDesign().hoverEffect === opt.value
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-5">
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-2">
                            角丸（px）: {getGlobalMenuDesign().borderRadius}
                          </label>
                          <input
                            type="range"
                            min="0"
                            max="48"
                            value={getGlobalMenuDesign().borderRadius}
                            onChange={e => updateGlobalMenuDesign('borderRadius', Number(e.target.value))}
                            className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-green-500"
                          />
                          <div className="flex justify-between text-[10px] text-gray-400 mt-1">
                            <span>0</span><span>48</span>
                          </div>
                        </div>
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-2">
                            項目間の隙間（px）: {getGlobalMenuDesign().gap}
                          </label>
                          <input
                            type="range"
                            min="0"
                            max="16"
                            value={getGlobalMenuDesign().gap}
                            onChange={e => updateGlobalMenuDesign('gap', Number(e.target.value))}
                            className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-green-500"
                          />
                          <div className="flex justify-between text-[10px] text-gray-400 mt-1">
                      ...
                      <div className="mb-5">
                        <label className="block text-xs font-medium text-gray-600 mb-2">
                          オーバーレイ濃度（%）: {getGlobalMenuDesign().overlayOpacity}
                        </label>
                        <input
                          type="range"
                          min="0"
                          max="100"
                          value={getGlobalMenuDesign().overlayOpacity}
                          onChange={e => updateGlobalMenuDesign('overlayOpacity', Number(e.target.value))}
                          className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-green-500"
                        />
                        <div className="flex justify-between text-[10px] text-gray-400 mt-1">
                          <span>0（透明）</span><span>100（不透明）</span>
                        </div>
                      </div>

                      {/* デフォルト背景グラデーション */}
                      <div className="mb-5">
                        <label className="block text-xs font-medium text-gray-600 mb-3">デフォルト背景グラデーション（画像なし時）</label>
                        <div className="grid grid-cols-2 gap-4">
                          <ColorPicker
                            label="開始色"
                            value={getGlobalMenuDesign().defaultGradientFrom}
                            onChange={v => updateGlobalMenuDesign('defaultGradientFrom', v)}
                          />
                          <ColorPicker
                            label="終了色"
                            value={getGlobalMenuDesign().defaultGradientTo}
                            onChange={v => updateGlobalMenuDesign('defaultGradientTo', v)}
                          />
                        </div>
                        <div
                          className="mt-2 h-8 rounded-lg"
                          style={{
                            background: `linear-gradient(to right, ${getGlobalMenuDesign().defaultGradientFrom}, ${getGlobalMenuDesign().defaultGradientTo})`,
                          }}
                        />
                      </div>
                      ...
                      {/* ラベル設定 */}
                      <div className="mb-5">
                        <label className="block text-xs font-medium text-gray-600 mb-3">ラベル設定</label>
                        <div className="grid grid-cols-3 gap-4">
                          <ColorPicker
                            label="テキスト色"
                            value={getGlobalMenuDesign().labelColor}
                            onChange={v => updateGlobalMenuDesign('labelColor', v)}
                          />
                          <div>
                            <FloatingInput
                              label="フォントサイズ（px）"
                              type="number"
                              value={getGlobalMenuDesign().labelFontSize}
                              onChange={v => updateGlobalMenuDesign('labelFontSize', v)}
                            />
                          </div>
                          <div>
                            <label className="block text-xs font-medium text-gray-600 mb-2">太さ</label>
                            <select
                              value={getGlobalMenuDesign().labelFontWeight}
                              onChange={e => updateGlobalMenuDesign('labelFontWeight', e.target.value)}
                              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900"
                            >
                              <option value="normal">標準</option>
                              <option value="medium">やや太め</option>
                              <option value="semibold">半太字</option>
                      ...
                      <div className="flex items-center gap-3">
                        <input
                          type="checkbox"
                          id="showInitialChar"
                          checked={getGlobalMenuDesign().showInitialChar}
                          onChange={e => updateGlobalMenuDesign('showInitialChar', e.target.checked)}
                          className="w-4 h-4 text-green-500 rounded border-gray-300 focus:ring-green-400"
                        />
                        <label htmlFor="showInitialChar" className="text-sm text-gray-700">
                          画像なし時に頭文字を表示する
                        </label>
                      </div>
                    </div>
