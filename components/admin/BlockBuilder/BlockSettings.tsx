  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto pr-6 pt-2">
        ...
        {block.type === 'row' && (
          <RowBlockSettings block={block} onUpdate={onUpdate} />
        )}
        {block.type === 'slider' && (() => {
          ...
          return (
            <div className="space-y-4">
              ...
              <CustomCheckbox
                label="自動スライド"
                checked={sliderConfig.autoplay ?? true}
                onChange={(checked) => onUpdate({ config: { ...sliderConfig, autoplay: checked } })}
              />
              {(sliderConfig.autoplay ?? true) && (
                <div className="-mt-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1 ml-7">
                    スライド間隔（秒）
                  </label>
                  <div className="ml-7">
                    <select
                      value={sliderConfig.autoplayInterval ?? 5}
                      onChange={(e) => onUpdate({ config: { ...sliderConfig, autoplayInterval: parseInt(e.target.value, 10) } })}
                      className="w-full px-3 py-2 rounded-lg border border-gray-300 bg-white text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      {[2, 3, 4, 5, 6, 7, 8, 10, 15].map(n => (
